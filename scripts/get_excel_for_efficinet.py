#!/usr/bin/env python3
"""
run_inference_and_export_efficientnet.py

— Given a directory of patient folders (each containing raw images), load a pretrained EfficientNet‐B0 classifier,
...
# 2) Path to your saved EfficientNet‐B0 checkpoint.
MODEL_PATH    = "/home/iambrink/NOH_Thyroid_Cancer_Data/Notebooks_folder/EB0_Test.pth"
# 3) Path where the final Excel file will be written.
OUTPUT_XLSX   = "/home/iambrink/NOH_Thyroid_Cancer_Data/TAN-TESTING/inference_results_efficientnet.xlsx"
...
def load_efficientnet_model(model_path: str) -> nn.Module:
    """
    1. Instantiate an EfficientNet-B0 with pretrained=True.
    2. Replace its classifier for 2 output classes.
    3. Load the state_dict from the checkpoint.
...
def run_inference_on_folder(model: nn.Module, transform: transforms.Compose, test_root: str):
    ...
    Walk through each patient subfolder under test_root. For each image:
      • apply transform
      • forward through EfficientNet → raw logits [1,2]
...
def main():
    ...
    # 2) Load the EfficientNet‐B0 checkpoint
